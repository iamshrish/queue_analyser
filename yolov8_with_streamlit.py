...
def load_yolov8_and_process_webcam(cap, confidence, stframe, kpi2_text,kpi3_text, time_interval):
    ...
    def get_embedding(cropped_head, id):
        try:
            t1 = time.time()
            print("t1", t1)
            # embedding = DeepFace.represent(img_path=cropped_head, enforce_detection=False)
            embedding = DeepFace.represent(img_path=cropped_head, model=vgg_face_model, enforce_detection=False)
            print("t2", time.time() - t1)
            if embedding is not None:
                collection.add(
                    embeddings=[embedding[0]["embedding"]],
                    documents=[f"Person_{id}"],
                    ids=[f"{id}"],
                )
                return embedding

        except (IndexError, ValueError) as e:
            print(f"Error extracting embedding for ID {id}: {e}")
            return None
        
   
    def compare_embeddings(new_embedding, collection):
    ...
    while True:
        ...
        for track_result in tracker_result:
            ...
            if id is None or id not in tracker_ids or not tracker_ids[id]:
                ...
                frame_count += 1
                embedding = get_embedding(cropped_head, id)
                if embedding is None:
                        continue

            if id is not None:
                filename = f"cropped_head_{id}.jpg"
                matched_id = compare_embeddings(embedding[0]["embedding"], collection)
...
# Update entry_exit_times for persons not present in the current_ids list
        for id in list(entry_exit_times.keys()):
            if id not in current_ids:
                entry_exit_times[id]["exit_time"] = time.time()
                waiting_time = entry_exit_times[id]["exit_time"] - entry_exit_times[id]["entry_time"]
