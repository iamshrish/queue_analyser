...
def special_draw(img, box, landmarsk, name, score=100):
    ...
    # draw bounding box
    cv2.rectangle(img, (int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1])), color, thickness=tl)
    # draw landmark
    ...
    cv2.rectangle(img, (int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1])), color, -1)  # filled
    cv2.putText(img, name, (int(c1[0]), int(c1[1] - 2)), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
